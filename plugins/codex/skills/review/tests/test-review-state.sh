...
if [ "\$1" = api ] && [ "\$2" = graphql ]; then
  ...
  if [ "\$FAKE_GH_MODE" = clean ]; then
    ...
    printf '%s\n' '{"data":{"repository":{"pullRequest":{
      ...
      "reactions":{"nodes":[]},
      "reviewThreads":{"nodes":[
        {"id":"PRRT_a","path":"src/a.ts","line":10,"isResolved":false,"comments":{"nodes":[{"id":"PRRC_a","author":{"login":"chatgpt-codex-connector"},"body":"**<sub><sub>![P1 Badge](https://img.shields.io/badge/P1-orange?style=flat)</sub></sub>  Fix the thing**\n\nDetail.\n\nUseful? React with 👍 / 👎."}]}},
        {"id":"PRRT_b","path":"src/b.ts","line":20,"isResolved":false,"comments":{"nodes":[{"id":"PRRC_b","author":{"login":"chatgpt-codex-connector"},"body":"**<sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub>  Nit the thing**\n\nDetail.\n\nUseful? React with 👍 / 👎."}]}}
      ]}
    }}}}'
  fi
  exit 0
...
assert "reviewed verdict carries the P1 as must-fix" check "len(d['must_fix']) == 1 and d['must_fix'][0]['file'] == 'src/a.ts' and d['must_fix'][0]['claim'] == 'Fix the thing'"
...
assert "reviewed verdict carries the P2 as a nit" check "len(d['nits']) == 1 and d['nits'][0]['file'] == 'src/b.ts'"
