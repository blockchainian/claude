...
# Synthetic bodies in the cloud reviewer's native P0/P1/P2 badge format.
cat > "$SCRATCH/threads.json" <<'EOF'
[
  {
    "thread_id": "PRRT_kwABC1",
    "comment_id": "PRRC_kwDEF1",
    "path": "src/services/priceFeedHub.ts",
    "line": 2210,
    "isResolved": false,
    "body": "**<sub><sub>![P1 Badge](https://img.shields.io/badge/P1-orange?style=flat)</sub></sub>  Limit subscriptions to active consumers**\n\nThis unconditionally opens a subscription for every incoming request.\n\nUseful? React with 👍 / 👎."
  },
  {
    "thread_id": "PRRT_kwABC2",
    "comment_id": "PRRC_kwDEF2",
    "path": "src/workers/activityPool.ts",
    "line": 118,
    "isResolved": false,
    "body": "**<sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub>  Allocate freed slots fairly across pools**\n\nPool redistribution can starve the primary pool of freed capacity.\n\nUseful? React with 👍 / 👎."
  },
  {
    ...
    "line": 1980,
    "isResolved": true,
    "body": "**<sub><sub>![P1 Badge](https://img.shields.io/badge/P1-orange?style=flat)</sub></sub>  Preserve intent for existing subscriptions**\n\nAlready fixed and resolved; must not resurface.\n\nUseful? React with 👍 / 👎."
  },
  {
    "thread_id": "PRRT_kwABC4",
    "comment_id": "PRRC_kwDEF4",
    "path": "src/hooks/useLiveData.ts",
    "line": 44,
    "isResolved": false,
...
assert "must-fix entry names the file" check "d['must_fix'][0]['file'] == 'src/services/priceFeedHub.ts'"
assert "must-fix entry keeps the line" check "d['must_fix'][0]['line'] == 2210"
assert "must-fix claim is the badge title, not the raw body" check "d['must_fix'][0]['claim'] == 'Limit subscriptions to active consumers'"
...
assert "unbadged unresolved thread defaults to nit" check "any(n['file'].endswith('useLiveData.ts') for n in d['nits'])"
assert "resolved P1 thread is dropped entirely" check "not any('Preserve intent' in (f.get('claim') or '') for f in d['must_fix'] + d['nits'])"
