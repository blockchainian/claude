...
```
1. Plan            — Claude (one turn): write spec.md + workstreams.txt, then launch
...
- **Artifact locations**: Claude (not codex) authors `spec.md` +
  ...
  `--spec` to reference them in prompts. The session that converts the plan
  also launches the run in the same turn; no handoff.md is written, since
  plan.md is the record a fresh session resumes from.
