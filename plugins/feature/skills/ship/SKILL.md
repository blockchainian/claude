...
7. **Triage, once.**
   ...
   e. `disposition` starts `fixed` for a finding you keep and `rejected` — with a one-line
      ...
      change, it misstates the code — quote the line that shows it, the failure it describes is
      provably impossible, or it is pure style): record the rejected ones here even though they
