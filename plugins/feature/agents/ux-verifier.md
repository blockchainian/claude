...
- iOS builds: `~/.claude/ux-verifier/<project>-sim-build` holds the commit of the last sim build (writing it is exempt from read-only). A rebuild is needed only when native inputs changed between that commit and the commit under test: `git diff --name-only <marker> <commit> -- <module>/ios <module>/app.json <module>/app.config.* <module>/package.json <module>/patches` prints something. Otherwise the installed dev-client is current and the commit's JavaScript is verified through a Metro reload with the project's reload helper (chadwallet: `sim_reload` in `scripts/ui-probes/lib.sh`), then `launch_app_sim` — never a build for a JS-only change, and never walk a stale build either way; the verdict would be about the wrong code. When a rebuild is needed, build the project's dev-client script from the main working tree (DerivedData is path-keyed; any new path = full cold build), never `clean`, reuse the same scheme/simulator via session defaults, write the marker after a successful build, and leave the sim booted with the app installed.
